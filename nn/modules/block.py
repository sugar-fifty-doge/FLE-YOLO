...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels(通道数变为原来的一半)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
...
class VoVGSCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):   #初始化参数
        super().__init__()     #调用父类的构造和方法，保证子类VoVGSCSP能够继承父类nn.Module的属性和方法
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.gsb = nn.Sequential(*(GSBottleneck(c_, c_, e=1.0) for _ in range(n)))  #一个包含多个GSBottleneck的序列，通过循环创建多个GSBottleneck层
